<x-register_employee.layout>
    ...
    <div class="my-12">
        <div class="flex justify-center items-center mb-6">
            <a href="{{ route('comments.my_comments', ['date' => $previousDate]) }}" class="text-4xl tracking-tighter font-medium text-red-500 mr-2">🢀</a>
            <h1 class="text-4xl tracking-tighter font-medium ">
               My Comments <b>{{ $currentDate }}</b>
            </h1>
            <a href="{{ route('comments.my_comments', ['date' => $nextDate]) }}" class="text-4xl tracking-tighter font-medium text-red-500 ml-2">🢂</a>
        </div>

        <hr/>
    </div>
    <form id="dateForm" action="{{ route('comments.my_comments') }}" method="GET">
